...
if (isset($_POST['trainer'])) {
	...
	if (empty($first)|| empty($email)|| empty($uid)|| empty($pwd)|| empty($gender)|| empty($phone)|| empty($admin)  ) 
	{
				   ...
				   if($resultCheck > 0) {
					 header("Location: ../trainer_signup.php?signup=usertaken");
	                  exit();   
					   ...
					   $sql = "INSERT INTO trainer (name,email, phone, traineruid, trainerpwd,gender,aid) VALUES ('$first', '$email','$phone', '$uid', '$pwd' ,'$gender','$admin')";
					   $result=mysqli_query($conn, $sql);
					   if ($result) {
